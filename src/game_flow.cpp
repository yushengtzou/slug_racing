      ...
      game_time(0)
{
    ...
    switch (state) {
        case EmployeeGame:
            line_count = 5;
            slugs[0] = std::make_unique<Slug>(renderer, display.get_slug_texture(AI_Wes), display.zoom_factor, AI_Wes, GameFlow::start_x, start_y, SlugState::MOVING, 1.0);
            ...
            slugs[2] = std::make_unique<Slug>(renderer, display.get_slug_texture(MM_Lulu), display.zoom_factor, MM_Lulu, GameFlow::start_x, start_y + interval_y * 2, SlugState::MOVING, 3.0);
            slugs[3] = std::make_unique<Slug>(renderer, display.get_slug_texture(MM_YuMin), display.zoom_factor, MM_YuMin, GameFlow::start_x, start_y + interval_y * 3, SlugState::MOVING, 1.0);
            slugs[4] = std::make_unique<Slug>(renderer, display.get_slug_texture(MM_MuHuan), display.zoom_factor, MM_MuHuan, GameFlow::start_x, start_y + interval_y * 4, SlugState::MOVING, 1.0);
            break;
        case InternGame:
            line_count = 2;
